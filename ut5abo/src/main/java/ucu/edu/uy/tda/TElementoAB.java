...
// tiene los dos hijos, buscamos el lexicograficamente anterior
    ...
    int contador <- 0 O(1)
	SI  hijoIzq <> nulo O(1)
		contador += this.hijoIzq.calcularCosto(FrecExito [],FrecNoExito [], indiceFE[], indiceFNE[] , nivel + 1) O(n)
	
SINO
contador += FrecNoExito[indiceFNE[0]] * nivel O(1)
indiceFNE++ O(1)
FIN SI

contador += FrecExito[indiceFE[0]] * nivel O(1)
indiceFE++  O(1)

SI hijoDer <> nulo O(1)
		contador += this.hijoDer.calcularCosto(FrecExito [],FrecNoExito [], indiceFE[], indiceFNE[] , nivel + 1) O(n)
SINO
		contador += FrecNoExito[indiceFNE[0]] * nivel O(1)
		indiceFNE++ O(1)
FIN SI
RETORNAR contador O(1)
 */
    @Override
