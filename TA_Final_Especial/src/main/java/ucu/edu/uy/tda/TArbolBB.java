...
//        return raiz.nivel(etiqueta, 0);
    ...
    public boolean cumpleAVL() {
        boolean[] esAVL = new boolean[]{true};
        if(!esVacio()){
            this.getRaiz().cumpleAVL(esAVL);
            return esAVL[0];
        }
        return esAVL[0];
    }
    ...
    @Override
    public Lista<Integer> paresPalabra(Comparable palabra) {
        Lista<Integer> listaPares = new Lista<>();
        if(this.getRaiz()!= null){
            this.getRaiz().paresPalabra(palabra, listaPares);
        }
        return listaPares;
    }
