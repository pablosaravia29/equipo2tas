...
public class TElementoAB<T> implements IElementoAB<T> {
    ...
    public void setListaEnteros(Lista<Integer> listaEnteros) {
...
// tiene los dos hijos, buscamos el lexicograficamente anterior
    ...
    public int cumpleAVL(boolean[] cumple) {
        ...
        if (this.hijoDer != null && cumple[0] == true) {
            alturaDer = this.hijoDer.altura();
        }
        if (Math.abs(alturaIzq - alturaDer) > 1) {
            cumple[0] = false;
        }
        return max(alturaIzq + 1, alturaDer + 1);
    }

    @Override
    public boolean paresPalabra(Comparable palabra, Lista<Integer> pares) {
        boolean valor = false;
        ...
        if (palabra.compareTo(this.getEtiqueta()) < 0) {
            if (this.getHijoIzq() != null) {
                valor = this.hijoIzq.paresPalabra(palabra, pares);
                if (valor == true) {
                    Nodo aux = this.getListaEnteros().getPrimero();
                    while (aux != null) {
                        if (Integer.parseInt((String) aux.getEtiqueta()) % 2 == 0) {
        ...
        if (palabra.compareTo(this.getEtiqueta()) > 0) {
            if (this.getHijoDer() != null) {
                valor = this.hijoDer.paresPalabra(palabra, pares);
                if (valor == true) {
                    Nodo aux = this.getListaEnteros().getPrimero();
                    while (aux != null) {
                        if (Integer.parseInt((String) aux.getEtiqueta()) % 2 == 0) {
